...
class NotificationRepo : public Observable {
private:
	vector<Tenant> repo;
public:
	/// <summary>
	/// adds a Tenant to notif list
	/// </summary>
	void store(const Tenant& apart) {
		for (const Tenant& t : repo)
			if (t == apart)
				throw RepoException("-The apartament is already in the notification list!\n");
		repo.push_back(apart);
		notify();
	}

	/// <summary>
	/// clears notiflist repo
	/// </summary>
	void destroy() { repo.clear(); notify(); }

	/// <summary>
	/// gets the vector of all notified tenants
	/// </summary>
	/// <returns>all notified tenants</returns>
	vector<Tenant> get_all() { return repo; }
};
