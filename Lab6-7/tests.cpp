...
void Tests::test_validator() const {
	...
	try {valid.validate_name("");}
	catch(ValidatorException){}
	
	try {valid.validate_name("OJFblkjkab^%$");} 
	catch (ValidatorException) {}

	try {valid.validate_name("aljn esf12efj");}
	catch (ValidatorException) {}

	valid.validate_apart(1253);
	try {valid.validate_apart(-1253);}
	catch (ValidatorException) {}

	valid.validate_type("ajhgih");
	try {valid.validate_type("");}
	catch (ValidatorException) {}

	try {valid.validate_type("OJFblkjkab^%$");}
	catch (ValidatorException) {}

	try {valid.validate_type("aljn esf12efj");}
	catch (ValidatorException) {}
	
	valid.validate_area(124);
	try {valid.validate_area(-1253);;}
	catch (ValidatorException) {}

	try {valid.validate_area(4);}
	catch (ValidatorException) {}

	try {valid.validate_area(0);}
	catch (ValidatorException) {}
...
void Tests::test_file_repository() const{
	try { FileRepository test("sdgdata.txt"); }
	catch(RepoException){}
...
void Tests::test_service() const {
	vector<Tenant> notif_list;
	InMemoryRepository trepo;
	Validator tvalid;
	Service tserv(trepo, tvalid);
	int n = 11, m = 10;
	...
	try {tserv.add_apartament_to_notification_list(notif_list, 12);}
	catch(ServiceException){}
	
	assert(notif_list.size() == 0);
	...
	m -= 2, n -= 2;
	try { tserv.add_tenant("nume", m, "tip", n); }
	catch(ServiceException) {}
	try { tserv.add_tenant("invalid", m, "%%%", n); }
	catch (ServiceException) {}
	...
	tserv.add_apartament_to_notification_list(notif_list, 11);
	assert(notif_list.size() == 1);
	tserv.add_apartament_to_notification_list(notif_list, 12);
	try { tserv.add_apartament_to_notification_list(notif_list, 12); }
	catch(ServiceException){}
	assert(notif_list.size() == 2);
	tserv.clear_notification_list(notif_list);
	assert(notif_list.empty());
	tserv.add_apartaments_randomly(notif_list, 2);
	tserv.add_apartaments_randomly(notif_list, 3);
	assert(notif_list.size() == 3);
	tserv.clear_notification_list(notif_list);

	tserv.edit_tenant("nume", m, "tip", n); 
	try { tserv.edit_tenant("numep", m, "tip", n); }
	catch (ServiceException) {} 
	try { tserv.edit_tenant("invalid", m, "%%%", n); }
	catch (ServiceException) {} 

	assert(tserv.find_apartament(m, "tip", n) == t);
	try { tserv.find_apartament(m, "tipqwtrydu", n); }
	catch (ServiceException) {}
	try { tserv.find_apartament(m, "%%%", n); }
	catch (ServiceException) {}
	...
	try { tserv.delete_tenant("numeweyt"); }
	catch (ServiceException) {}
	tserv.delete_tenant("nume");
	tserv.delete_tenant("numed");
	tserv.delete_tenant("numet");
	try { tserv.delete_tenant("nume"); }
	catch (ServiceException) {}
	try { tserv.delete_tenant("inv@lid"); }
	catch (ServiceException) {}
...
void Tests::test_export_to_files() const {
	...
	vector<Tenant> notif_list;
	notif_list.push_back(t1);
	notif_list.push_back(t2);
	notif_list.push_back(t3);
	notif_list.push_back(t4);
	Service::export_to_file(notif_list,"test.csv");
	...
	Service::export_to_file(notif_list, "test.html"); 
